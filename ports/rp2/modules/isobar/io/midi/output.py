...
class MidiOutputDevice (OutputDevice):
    def __init__(self, device_name=None, send_clock=False, virtual=False):
        ...
        try:
            if device_name is None:
                device_name = "foo" #dg os.getenv("ISOBAR_DEFAULT_MIDI_OUT")
            self.midi = None#mido.open_output(device_name, virtual=virtual)
        except (RuntimeError, OSError):
            raise DeviceNotFoundException("Could not find MIDI device")
        self.send_clock = send_clock
        log.info("Opened MIDI output: foo")
    ...
    def start(self):
        ...
        """
        if self.send_clock:
            msg = None#mido.Message("start")
            print("midi send") #self.midi.send(msg)
    ...
    def tick(self):
        if self.send_clock:
            msg = None#mido.Message("clock")
            print("midi send") #self.midi.send(msg)

    def note_on(self, note=60, velocity=64, channel=0):
        print("[midi] Note on  (channel = %d, note = %d, velocity = %d)" % (channel, note, velocity))
        log.debug("[midi] Note on  (channel = %d, note = %d, velocity = %d)" % (channel, note, velocity))
        msg = None#mido.Message('note_on', note=int(note), velocity=int(velocity), channel=int(channel))
        print("midi send") #self.midi.send(msg)

    def note_off(self, note=60, channel=0):
        log.debug("[midi] Note off (channel = %d, note = %d)" % (channel, note))
        msg = None#mido.Message('note_off', note=int(note), channel=int(channel))
        print("midi send") #self.midi.send(msg)
    ...
    def control(self, control=0, value=0, channel=0):
        ...
        msg = None#mido.Message('control_change', control=int(control), value=int(value), channel=int(channel))
        print("midi send") #self.midi.send(msg)

    def program_change(self, program=0, channel=0):
        log.debug("[midi] Program change (channel %d, program_change %d)" % (channel, program))
        msg = None#mido.Message('program_change', program=int(program), channel=int(channel))
        print("midi send") #self.midi.send(msg)

    def pitch_bend(self, pitch=0, channel=0):
        log.debug("[midi] Pitch bend (channel %d, pitch %d)" % (channel, pitch))
        msg = None#mido.Message('pitchwheel', pitch=int(pitch), channel=int(channel))
        print("midi send") #self.midi.send(msg)

    def set_song_pos(self, pos=0):
        msg = None#mido.Message('songpos', pos=pos)
        print("midi send") #self.midi.send(msg)
